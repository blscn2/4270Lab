...
void MEM()
{
	...
	if(EX_MEM.IR != 0 ){
		...
		int miss=0;
		uint32_t b =MEM_WB.ALUOutput&0x0003;
		uint32_t w =(MEM_WB.ALUOutput&0x000C)>>2;
		uint32_t B =(MEM_WB.ALUOutput&0x00F0)>>4;
		uint32_t t =(MEM_WB.ALUOutput&0xFF00)>>8;
		int sub = w;
		CacheBlock writeBuffer;

		if(t != L1Cache.blocks[B].tag){
			miss=1;
		}else if( (L1Cache.blocks[B].valid==0)){
			miss=1;
		}
		...
		switch(EX_MEM.Control){
			case LOAD_TYPE: 
				if(miss==1){	
					...
					for(int i=0; i<4; i++){
						L1Cache.blocks[B].words[i]=mem_read_32(EX_MEM.ALUOutput-sub);
						sub-=4;
					}
					L1Cache.blocks[B].valid=1;
					L1Cache.blocks[B].tag=t;
					cache_misses+=1;
				}
				else{
					/*
					switch(MEM_WB.num_bytes){					
						case BYTE:
							MEM_WB.LMD = (uint32_t)(0x00FF) & L1Cache.blocks[B].words[i];	
						break;
						case HALF_WORD:
							MEM_WB.LMD = (uint32_t)(0xFFFF) & L1Cache.blocks[B].words[i];	
						break;
						case WORD:	
							MEM_WB.LMD = L1Cache.blocks[B].words[i];
			...
			case STORE_TYPE: 
				if(miss==1){	
					...
					switch(EX_MEM.num_bytes){
						case BYTE:		
							L1Cache.blocks[B].words[w]=0x00FF & EX_MEM.B; 		
						break;
						case HALF_WORD: 
							L1Cache.blocks[B].words[w]=0xFFFF & EX_MEM.B; 		
						break;			
						case WORD: 		
							L1Cache.blocks[B].words[w]=EX_MEM.B; 				
						break;
						default: 												
						break;				
					}
