...
mips_instr_t mips_instr_decode( uint32_t instr )
{
	...
	return instr_info;
	/*
	uint32_t instruct = mem_read_32( instr);
//	uint32_t opcode = instruct >> 26;
	//printf("\nopcode, 0x%02x\n", opcode);
	uint32_t s_opcode = instruct & 0x3F; //get's special op;
