...
uint8_t checkDataHazard()
{
    ...
    // check execution for data hazard
...
//see if forwarding is possible
uint8_t checkForward()
{
    uint8_t dest;
    
	uint8_t bMultDivForward = ((EX_MEM.instr_data.opcode = 0x00) 
	&& (((EX_MEM.instr_data.funct_code >= 0x18) 
	&& (EX_MEM.instr_data.funct_code <= 0x1B))
	|| (EX_MEM.instr_data.funct_code == 0x11) 
	|| (EX_MEM.instr_data.funct_code == 0x13)));

	// check execution for hazard
	if(!bMultDivForward) {
		if( EX_MEM.instr_data.type == R_TYPE )
		{
		    dest = GET_RD( EX_MEM.IR );
	...
	&& (((MEM_WB.instr_data.funct_code >= 0x18) 
	&& (MEM_WB.instr_data.funct_code <= 0x1B)) 
	|| (MEM_WB.instr_data.funct_code == 0x11) 
	|| (MEM_WB.instr_data.funct_code == 0x13)));

    // check memory for hazard
	if(!bMultDivForward) {
		if( MEM_WB.instr_data.type == R_TYPE )
		{
		    dest = GET_RD( MEM_WB.IR );
